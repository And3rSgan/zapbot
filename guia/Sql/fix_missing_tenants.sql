-- ============================================
-- PASSO 1: DIAGNÓSTICO - Rode isso primeiro para ver o estado
-- ============================================
SELECT 
  au.id as user_id, 
  au.email,
...
LEFT JOIN public.tenants t ON t.id = COALESCE(p.tenant_id, tm.tenant_id)
...
-- ============================================
-- PASSO 3: VERIFICAÇÃO - Rode depois para confirmar
-- ============================================
SELECT 
  au.id as user_id, 
  au.email,
...
-- Verificar se existe um plano ativo
SELECT id, name, price_cents, trial_days, active FROM public.plans ORDER BY price_cents ASC;
